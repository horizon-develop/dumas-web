...
const SlidableBanner: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const totalWidth = children.reduce((acc, child) => 
      acc + child.getBoundingClientRect().width + 8, 0);
    ...
    const animation = content.animate(
      ...
      {
        duration: totalWidth * (window.innerWidth < 768 ? 40 : 50),
        iterations: Infinity
      }
    );
  ...
  return (
    <div className="bg-[#8B0000] py-8 md:py-12 overflow-hidden w-full">
      ...
      <div ref={containerRef} className="relative w-full overflow-hidden">
        <div
          ref={contentRef}
          className="flex w-fit gap-2 md:gap-4 px-1 md:px-2 overflow-x-auto"
        >
          {images.map((img, index) => (
            <div 
              key={index}
              className="flex-shrink-0 w-[200px] h-[114px] md:w-[320px] md:h-[183px] rounded-lg md:rounded-xl overflow-hidden shadow-md active:shadow-lg md:hover:shadow-lg transition-shadow duration-300"
            >
              <img
                src={img}
